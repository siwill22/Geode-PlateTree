...
import { PlateTreeUi, type PlateTreeViewState } from './plateTreeUi';
...
interface Manifest {
  ...
  static_polygons: { geometry: string; rotations: string; plate_names?: string };
  plate_tree: string;
...
async function boot(): Promise<void> {
  ...
  const [coastData, polys, treeBytes] = await Promise.all([
